...
# `HRVO Library` Quality Declaration

The package `HRVO Library` claims to be in the **Quality Level 2** category.
...
`HRVO Library` uses [Semantic Versioning 2.0.0](https://semver.org/), and is
at version `1.1.0`. The version is declared in `CMakeLists.txt` and
`MODULE.bazel`.

### Version Stability [1.ii]

`HRVO Library` is at a stable version (`1.1.0`), which is greater than
`1.0.0`.

### Public API Declaration [1.iii]

The public API of `HRVO Library` is the set of symbols declared in
[`src/HRVO.h`](src/HRVO.h). This header aggregates the public interfaces of
`Simulator` and `Vector2`. Internal implementation details in `Agent`, `Goal`,
and `KdTree` are not part of the public API.
...
All changes to `HRVO Library` are submitted through pull requests on
[GitHub](https://github.com/snape/HRVO). Direct commits to the `main` branch
...
- [`.github/workflows/ci.yml`](.github/workflows/ci.yml): builds and tests
  ...
  openSUSE, and Ubuntu (amd64 and arm64), and macOS (arm64), and with CMake on
  Windows (amd64). CMake builds enable `BUILD_TESTING`, `ENABLE_HARDENING`,
...
All features of `HRVO Library` are documented in the Doxygen markup within
[`src/HRVO.h`](src/HRVO.h) and related headers, including a usage guide,
...
This document is the quality declaration for `HRVO Library` and is linked from
the project README.
...
The Circle simulation scenario exercises the major public API functions of
`Simulator` and `Vector2`. Dedicated unit tests for individual API functions
are not currently present.

### Coverage [4.iii]

Code coverage is not currently tracked. The Circle simulation scenario provides
...
No formal performance regression tests are in place. The Circle simulation
scenario implicitly exercises performance characteristics of the library (e.g.,
with 250 agents), but no automated performance benchmarks are run in CI.
...
`HRVO Library` is built and tested continuously via GitHub Actions on the
following platforms:
